"""Create CrySyS subset from S-series dirs (~5M frames)."""
import sys, json
import numpy as np
import pandas as pd
...
print("Saved frames.parquet")

# Build window index
WINDOW, STRIDE = 100, 100
windows = []
for sp in [SPLIT_TRAIN, SPLIT_VAL, SPLIT_TEST]:
    sp_mask = sub.split.values == sp
    sp_idxs = np.where(sp_mask)[0]
...
for sp, nm in [(SPLIT_TRAIN, "train"), (SPLIT_VAL, "val"), (SPLIT_TEST, "test")]:
    ...
    print(f"  {nm}: {len(sw)} windows, {sw[:, 2].sum()} attack")
...
# Train stats
can_ids = sub["can_id"].values
train_mask = sub["split"].values == SPLIT_TRAIN
unique_ids = sorted(set(can_ids[train_mask].tolist()))
train_stats = {
...
print("Saved train_stats.json")

# Splits json (frame index -> split)
splits = {str(i): int(v) for i, v in enumerate(split_col) if v >= 0}
with open(DST / "splits.json", "w") as f:
    json.dump(splits, f)
print(f"Saved splits.json ({len(splits)} entries)")

print(f"\n=== DONE: CrySyS subset at {DST} ===")
print(f"=== {len(sub)} frames, ~{len(sub)*8/1e6:.0f}MB in memory ===")
